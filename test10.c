  ...
  for (int mapZ = 0; mapZ < mapDepth; mapZ++) {
    for (int mapX = 0; mapX < mapHeight; mapX++) {
      for (int mapY = 0; mapY < mapWidth ; mapY++) {
        ...
        if (collision && IsMouseButtonPressed(MOUSE_BUTTON_RIGHT)) {
...
// Allocates the space for a 2D map
int** mapAllocator(int width, int height){
  int **map;
...
int main(void)
{
  ...
  int mapWidth = 20, mapHeight = 20, mapDepth = 3;
